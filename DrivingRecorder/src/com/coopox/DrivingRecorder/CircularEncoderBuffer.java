...
public class CircularEncoderBuffer {
    ...
    public void add(ByteBuffer buf, int flags, long ptsUsec) {
        ...
        if (VERBOSE) {
            Log.d(TAG, "add chunk flags=0x" + Integer.toHexString(flags) +
                    " pts=" + ptsUsec);
        }
        while (!canAdd(size)) {
            removeTail();
        ...
        if (VERBOSE) {
            Log.d(TAG, String.format("Add offset = %d, Size = %d", packetStart, size));
        }
    ...
    public void add(ByteBuffer buf, MediaCodec.BufferInfo info) {
        int size = info.size;
        if (VERBOSE) {
            Log.d(TAG, "add info chunk flags=0x" + Integer.toHexString(info.flags) +
        ...
        mPacketPtsUsec[mMetaHead] = info.presentationTimeUs;
        mPacketStart[mMetaHead] = packetStart;
        mPacketLength[mMetaHead] = size;
        if (VERBOSE) {
            Log.d(TAG, String.format("Add offset = %d, Size = %d", packetStart, size));
        ...
        if (packetStart + size < dataLen) {
            // one chunk
            System.arraycopy(buf.array(), info.offset, mDataBuffer, packetStart, size);
//            buf.get(mDataBuffer, packetStart, size);
        } else {
            // two chunks
            int firstSize = dataLen - packetStart;
...
//            buf.get(mDataBuffer, packetStart, firstSize);
//            buf.get(mDataBuffer, 0, size - firstSize);
            System.arraycopy(buf.array(), info.offset, mDataBuffer, packetStart, firstSize);
            System.arraycopy(buf.array(), info.offset + firstSize, mDataBuffer, 0, size - firstSize);
        }
    ...
    /**
     * Returns the index of the last sync frame.  Valid until the next add().
     */
    public int getLastIndex() {
        ...
        int index = mMetaHead - 1;
        while (index != mMetaTail) {
            if ((mPacketFlags[index] & MediaCodec.BUFFER_FLAG_SYNC_FRAME) != 0) {
                break;
            }
            index = (index - 1) % metaLen;
    ...
    public ByteBuffer getChunk(int index, MediaCodec.BufferInfo info) {
        ...
        if (packetStart + length <= dataLen) {
            ...
            if (VERBOSE) {
                Log.v(TAG, String.format("Will allocate %d bytes buffer", length));
            }
            // two chunks
    ...
    public void setTailIndex(int index) {
        mMetaTail = index;
    }
}
